...
- Customize Snake: free Cyan, Electric Blue, Violet, Magenta, Mint, Teal, Gold, Pearl glow. The owner’s September 6 follow-up supersedes the original fixed-cyan-head requirement: head and body now share the selected glow. Retain the recognizable head silhouette and distinct hostile shapes. One existing renderer powers live preview with rotation/zoom, Apply/Cancel/Restore Cyan, keyboard/controller operation, title/pause entry and independent persistent appearance. Earned finishes/trails remain future progression work.
...
## Owner appearance and access revision — September 6, 2026

The owner’s screenshots showed muted customization colors and an undersized Lab shortcut. The revision strengthens and widens the player’s emissive light strips, increases palette saturation, removes the preview’s dark overlay and applies the selected glow to head lights and the small direction marker as well as the body. The armored silhouette, rival red faction, collision dimensions, simulation statistics and saved appearance IDs remain unchanged. The earlier fixed-cyan-head design is explicitly superseded by this owner request.

POWERUP LAB is a primary title-menu entry immediately below START GAME, with the same button size and keyboard/controller behavior. The briefing shortcut remains. Direct title access preserves suspended campaign data and returns to the title when closed. Short layouts retain reachable menu controls, including CONTINUE RUN.
