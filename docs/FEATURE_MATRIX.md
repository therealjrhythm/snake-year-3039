...
Owner fix 0.3.2 restores held stick/D-pad/keyboard steering on the first active frame after Warden instructions, countdowns and resume while retaining action release gates and menu/blur protection. Current collection contact is 0.18 units more forgiving for cores, powers and numbered spheres, with scenery obstruction and all hostile/crash boundaries preserved; older content keeps its original radii. **101 simulation tests** and the [Warden handoff report](evidence/warden-controls-2026-09-07/warden-controls-report.json) document bounded behavior; [sequential verification](evidence/warden-controls-2026-09-07/verification.json) records the final gates. Physical Xbox confirmation remains open.
...
| INP-01–06 | Shared keyboard/Xbox actions, menu focus, device ownership/glyphs, remapping, dead zone, immediate steering/action isolation on transitions, disconnect recovery | Implemented shared actions plus explicit remembered menu selection, embedded-focus recovery, form-row navigation and custom option lists; held steering resumes immediately after gameplay countdowns; full mocked controller menu and prepared Warden handoff flows pass; hardware/remapping incomplete | Full keyboard flow plus physical Xbox tests; remapping/recovery still required; B/D/E |
...
| COL-01–07 | Full head/body/hazard/exit collision table, critical-crash priority, equal-time ordering, swept collision, neck exclusion, contact deduplication | Implemented D1 foundation; swept collection margin and solid-obstruction checks tested for current content; full requirement verification open | Each collision row and simultaneous ordering tested; A/B/E |
...
Current build/content: **0.3.2 / 0.3.0-neon-spire**, September 7, 2026. Earlier 70-test, hybrid/pad and glow evidence below is historical unless explicitly rerun for this revision. Browser reports contain exact timestamps and Chrome 152.0.7977.77 metadata. All scripts use isolated installed-Chrome profiles through Playwright because the Browser plugin is unavailable. They run sequentially and do not alter the owner's browser profile. Local diagnostic frame observations do not qualify a machine or physical Xbox connection.

| Evidence | Result and bounded coverage | Limitation |
| --- | --- | --- |
| [0.3.2 Warden controls](evidence/warden-controls-2026-09-07/warden-controls-report.json), [pickup tests](../tests/pickup-contact.test.ts), [verification](evidence/warden-controls-2026-09-07/verification.json) | 101 tests and production build pass. 28 mapper assertions cover immediate held steering, suppressed actions, fresh-action release, neutral input and blur; seven browser groups cover actual final-core/Warden popup, countdown, inward turn and pause/resume with stick, D-pad and keyboard. Thirteen pickup regressions cover boosted grazing, just-outside misses, solid obstruction, full-slot exclusions, relay order, crash priority and old-version/save behavior. | Prepared saves and simulated controls, not physical Xbox or an ordinary full district clear. Before-fix mapper failure and the existing systems fixture adjustment are retained alongside final results. |
