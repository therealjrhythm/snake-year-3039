...
export function Settings({ value, onChange, onClose }: { value: SettingsValue; onChange: (v: SettingsValue) => void; onClose: () => void }) {
  ...
  return <Modal title="SETTINGS" subtitle="CUSTOMIZE YOUR EXPERIENCE" onClose={onClose} className="settings-dialog">
    ...
    <div className="settings-content" role="tabpanel" aria-label={tab}>
      ...
      {tab === 'Visuals' ? <><h3>REFLECTED IN NEON</h3><label className="setting-row"><span>Graphics quality</span><select value={value.quality} onChange={e => update('quality', e.target.value as SettingsValue['quality'])}><option value="low">Low</option><option value="medium">Medium</option><option value="high">High</option></select></label><Range label="Bloom strength" value={value.bloom} onChange={n => update('bloom', n)} /><p className="settings-note">Quality changes render resolution and atmosphere. Collision geometry and warnings stay consistent.</p></> : null}
