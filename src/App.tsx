...
export default function App() {
  ...
  useEffect(() => { if (!overlay) requestAnimationFrame(() => (document.querySelector<HTMLElement>('[data-menu] [data-autofocus]') ?? document.querySelector<HTMLElement>('[data-menu] button'))?.focus()); }, [screen, overlay]);
  ...
  return <main className={`app ${settings.highContrast ? 'high-contrast' : ''} ${settings.reducedMotion ? 'reduced-motion' : ''}`} style={{ '--ui-scale': settings.uiScale } as React.CSSProperties}>
    ...
    {screen === 'briefing' ? <div className="briefing panel" data-menu inert={overlay !== null}><button className="text-button" onClick={() => changeScreen('title')}><ArrowLeft size={16} />BACK</button><div className="briefing-number">01</div><span className="hud-label">CAMPAIGN / FIRST DISTRICT</span><h2>NEON SPIRE</h2><p>The city closed its energy network.<br />You were built to open it.</p><div className="briefing-stats"><div><strong>03</strong><span>COLLECTION WAVES</span></div><div><strong>36</strong><span>ENERGY CORES</span></div><div><Radio size={29} /><span>WARDEN FINALE</span></div></div><label className="difficulty-label">RULES PROFILE<select value={difficulty} onChange={e => setDifficulty(e.target.value as Difficulty)}><option value="standard">Standard · 3 integrity</option><option value="assisted">Assisted · 5 integrity · slower world</option><option value="expert">Expert · faster hostile projectiles</option></select></label><p className="briefing-tip">You move continuously. Steer with {device === 'keyboard' ? 'WASD or arrows' : 'the left stick or D-pad'}. Your shield stops attacks; walls and your own body still cause a critical crash.</p>{hasSave ? <p className="replace-note">Starting a new run replaces your suspended run.</p> : null}<button className="menu-button primary" onClick={() => start('campaign')} data-autofocus><Play size={20} fill="currentColor" />ENTER NEON SPIRE<ArrowRight size={20} /></button><button className="text-button practice-button" onClick={() => start('practice')}>Practice without records <ArrowRight size={15} /></button></div> : null}
